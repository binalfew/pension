...
export async function saveSearchHistory(
  ...
  try {
    await prisma.$executeRaw`
      MERGE search_history AS target
      USING (SELECT ${adminEmail} AS admin_email, ${sapId} AS sap_id, ${employeeName} AS employee_name, GETDATE() AS searched_at) AS source
      ON (target.admin_email = source.admin_email AND target.sap_id = source.sap_id)
      WHEN MATCHED THEN
        UPDATE SET 
          employee_name = source.employee_name,
          searched_at = source.searched_at
      WHEN NOT MATCHED THEN
        INSERT (admin_email, sap_id, employee_name, searched_at)
        VALUES (source.admin_email, source.sap_id, source.employee_name, source.searched_at);
    `;
  } catch (error) {
    // If table doesn't exist, just log the error and continue
    console.warn("Search history table not found, skipping save:", error);
  }
}

// Get recent search history for admin
export async function getSearchHistory(adminEmail: string): Promise<
  Array<{
    sapId: number;
    employeeName: string;
    searchedAt: Date;
...
> {
  try {
    ...
    >`
      SELECT TOP 10 sap_id as sapId, employee_name as employeeName, searched_at as searchedAt
      FROM search_history 
    ...
    console.warn(
      "Search history table not found, returning empty history:",
      error
    );
    return [];
  }
